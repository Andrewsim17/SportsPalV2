...
export const communitiesApi = {
    ...
    const { data, error } = await supabase
      ...
      .select(`
        *,
        ...
        discussions:community_discussions(*, user:profiles(id, name, avatar_url))
      `)
      .eq('id', communityId)
      .single();
  ...
  // Add function to create a new discussion
  createDiscussion: async (discussionData, userId) => {
    try {
      ...
      if (!title || !content || !communityId || !userId) {
        throw new Error('Missing required fields: title, content, communityId, userId');
      }
      
      // Step 1: Insert the discussion
      const { data: newDiscussion, error: insertError } = await supabase
        .from(TABLES.COMMUNITY_DISCUSSIONS)
        .insert({
          title,
          content,
          community_id: communityId,
      ...
      // Step 2: If there are attachments, insert them
      if (attachments && attachments.length > 0) {
        ...
        if (attachmentsError) {
          ...
          // Continue despite attachment error
        }
      }
      
      // Step 3: If there's a poll, insert the poll options
      if (poll && poll.options && poll.options.length >= 2) {
        ...
        if (formattedOptions.length >= 2) {
          ...
          if (pollError) {
            console.error('Error adding poll options:', pollError);
            // Continue despite poll error
          }
        }
      }
  ...
  getCommunityDiscussion: async (discussionId) => {
    const { data, error } = await supabase
      .from(TABLES.COMMUNITY_DISCUSSIONS)
      .select(
        '*,' +
  ...
  getDiscussionReplies: async (discussionId) => {
    try {
      // Step 1: Get all replies for the discussion
      const { data: replies, error: repliesError } = await supabase
        .from(TABLES.DISCUSSION_REPLIES)
        .select('*')
      ...
      // Step 2: Get all the user profiles needed
      ...
      // Step 3: Combine the replies with their corresponding user profiles
      const repliesWithUsers = replies.map(reply => ({
        ...reply,
        user: profileMap[reply.user_id] || null
      }));
  ...
  addReplyToDiscussion: async (discussionId, userId, content) => {
    try {
      ...
      // Step 3: Update the replies count in the discussion
      try {
        await supabase.rpc('increment_discussion_replies_count', {
          discussion_id_param: discussionId,
        });
      } catch (countError) {
        console.error('Failed to update replies count:', countError);
      }
      ...
      console.error('Error in addReplyToDiscussion:', error);
      throw error;
    }
  },
  ...
  likeReply: async (replyId, userId) => {
    try {
      ...
      // If the user has already liked this reply, don't do anything
      if (existingLike) {
  ...
  unlikeReply: async (replyId, userId) => {
    try {
      // Delete the like
      const { error } = await supabase
        .from('reply_likes')
  ...
  checkIfUserLikedReply: async (replyId, userId) => {
    ...
    try {
      ...
      return !!data; // Return boolean indicating if the user liked the reply
    } catch (error) {
  ...
  likeDiscussion: async (discussionId, userId) => {
    try {
      ...
      // If the user has already liked this discussion, don't do anything
      if (existingLike) {
        return { alreadyLiked: true };
      }
      ...
      // Update the likes count in the discussion
      try {
        await supabase.rpc('increment_discussion_likes_count', {
          discussion_id_param: discussionId,
        });
      } catch (countError) {
        console.error('Failed to update likes count:', countError);
      }
  ...
  unlikeDiscussion: async (discussionId, userId) => {
    try {
      // Delete the like
      const { error } = await supabase
        .from('discussion_likes')
      ...
      // Update the likes count in the discussion
      try {
        await supabase.rpc('decrement_discussion_likes_count', {
          discussion_id_param: discussionId,
        });
      } catch (countError) {
        console.error('Failed to update likes count:', countError);
  ...
  checkIfUserLikedDiscussion: async (discussionId, userId) => {
    if (!userId) return false;
    ...
    try {
      ...
      return !!data; // Return boolean indicating if the user liked the discussion
    } catch (error) {
